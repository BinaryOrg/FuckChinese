...
@interface HaHaCommentListController ()
...
@implementation HaHaCommentListController
...
- (void)setModel:(HaHaDuanziModel *)model {
    _model = model;
    [self headerRefresh];
}
...
- (void)headerRefresh {
    ...
    [MFNETWROK post:@"Comment/ListCommentByTargetid" params:@{@"targetId" : self.model.id} success:^(id result, NSInteger statusCode, NSURLSessionDataTask *task) {
        [MFHUDManager dismiss];
        if (statusCode == 200) {
            self.dataArr = [NSArray yy_modelArrayWithClass:HaHaCommentModel.class json:result[@"data"]];
            [self.tableNode reloadData];
        }else {
...
- (ASCellNodeBlock)tableNode:(ASTableNode *)tableNode nodeBlockForRowAtIndexPath:(NSIndexPath *)indexPath {
    return ^ASCellNode *() {
        if (indexPath.section == 0) {
            HaHaSecondListCellNode *node = [[HaHaSecondListCellNode alloc] initWithModel:self.model];
            return node;
        }
